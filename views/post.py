...
from models import Post, PendingPost, ReportedPost
from forms import PostForm, ReportForm, PostReplyForm
from database import insertSimple, activatePendingPost, getUserPost, reactivatePost, getPostDetails, insertNewPostAsPending, databaseRollback
from goldeeEmail import sendEmailNewPost, sendEmailNewPostReply
...
def newPost():
	...
	if form.validate_on_submit(): # if Post request and all fields validate
		...
		somewhat secure
		'''
		# Once app has user persistence, can remove PendingPost and keep activation behind @login_required so only
		# the user can change the post's properties (i.e. activate/reactivate/edit).
		try:
			postHash = insertNewPostAsPending(post)
			postLink = "http://goldee-env.us-west-1.elasticbeanstalk.com/post/new/" + postHash
...
def reportPost(postID):
	...
	if form.validate_on_submit():
		...
		try:
			...
			flash("We're sorry, something went wrong. Please try again.")
			return redirect('/' + str(postID) + '/report') 
